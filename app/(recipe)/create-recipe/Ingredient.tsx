...
export default function Ingredient({ recipe, setRecipe }: IngredientProp) {
  ...
  const handleNameChange = useCallback(
    (value: string, index: number) => {
      setRecipe((prev) => {
        const currentIngres = prev.ingredients ?? [];
        const next = [...currentIngres];
        if (next[index]) {
          next[index] = { ...next[index], name: value };
        }
        return {
  ...
  const deleteThisIngre = (index: number) => {
    if (ingredients.length <= 1) return;
    updateIngredients(ingredients.filter((_, idx) => idx !== index));
  };
    ...
    <div className="w-full mb-6 text-left">
      ...
      <div className="flex flex-col gap-2.5">
        {ingredients.map((ingre, inx) => (
          <div key={inx} className="flex items-center gap-2 w-full">
