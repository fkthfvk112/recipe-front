...
import type { EvtCondition } from "./RegisterEvt";
...
export default function UpdateEvt({eventId, setPage, setSelectedEventId}:EventUpdProps) {
    ...
    useEffect(() => {
        if (eventId && eventId != 0) {
            axiosAuthInstacne.get(`/admin/evt/${eventId}`).then(res => {
                const data = res.data;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        axiosAuthInstacne.put(`/admin/evt/update/${eventId}`, payload)
            .then((res) => {
